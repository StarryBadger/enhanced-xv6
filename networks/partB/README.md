...
### Retransmission:
#### UDP-based Implementation
In my implementation, retransmission of lost or unacknowledged packets is handled using the following strategy: The sender uses a non-blocking recvfrom() to wait for 20 microseconds for ACK to be received after sending the packet. The acknowledgement for a packet is simply the sequence number corresponding to it. During the packet transmission, we store the time when the packet was last sent.

After transmitting each packet we check if we have received the acknowledgement for all previously transmitted packets. If not, we check if the time of such a packet being last sent has exceeded 0.1 second, in which case we retransmit the packet. After the final pcket has been sent (atleast once), we keep trying to resend unacknowledged packets by the above procedure until ACK is received for all packets. This approach requires us to manage retransmission logic explicitly.

After all packets are received, the receiver arranges the packets in increasing order of sequence numbers to construct the data sent.
#### Actual TCP
